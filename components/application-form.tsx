...
export function ApplicationForm() {
      ...
      if (!response.ok || !result.ok) {
        ...
        const localizedErrors = Object.fromEntries(
          Object.keys(serverErrors)
            .filter((key) => key !== "form")
            .map((key) => [key, locale === "en" ? serverErrors[key] : c.review]),
        );
  ...
  return (
    <form className="application-form" onSubmit={handleSubmit} noValidate aria-busy={submission.kind === "submitting"}>
      ...
      <button className="button button--primary form-submit" type="submit" disabled={submission.kind === "submitting"}>
        <span>{submission.kind === "submitting" ? c.submitting : c.submit}</span>
